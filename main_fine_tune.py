...
def parse_option():
    ...
    parser.add_argument('--model_dir', type=str, default='./save/finetune',
    ...
    parser.add_argument('--propmt_pth', type=str, default=None,
                        help='prompt path to load from checkpoint')
    ...
    parser.add_argument('--use_prompter', default=True,
                        action="store_false",
                        help='whether to use prompter')
    args = parser.parse_args()

    args.filename = 'finetune_{}_{}_{}_{}_{}_lr_{}_decay_{}_bsz_{}_trial_{}_prompter_{}'. \
        format(args.method, args.prompt_size, args.dataset, args.model,
               args.optim, args.learning_rate, args.weight_decay, args.batch_size, args.trial,args.use_prompter)
...
def main():
    ...
    if args.propmt_pth:
        if os.path.isfile(args.propmt_pth):
            if args.gpu is None:
                print("=> loading checkpoint '{}'".format(args.propmt_pth))
            if args.gpu is None:
                checkpoint = torch.load(args.propmt_pth)
            else:
                # Map model to be loaded to specified single gpu.
                loc = 'cuda:{}'.format(args.gpu)
                checkpoint = torch.load(args.propmt_pth, map_location=loc)
            args.start_epoch = checkpoint['epoch']
            best_acc1 = checkpoint['best_acc1']
            if args.gpu is not None:
                # best_acc1 may be from a checkpoint from a different GPU
            ...
            print("=> no checkpoint found at '{}'".format(args.propmt_pth))
    prompter.eval()
    # create data
