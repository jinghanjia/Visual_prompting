...
def parse_option():
    ...
    parser.add_argument('--model', type=str, default=None,
                        choices=['rn50', 'instagram_resnext101_32x8d', 'bit_m_rn50','cifar100_res20'],
                        help='choose pre-trained model')
    parser.add_argument('--method', type=str, default='padding_class',
                        choices=['padding', 'random_patch', 'fixed_patch','padding_class'],
...
def main():
    ...
    elif args.model == 'cifar100_res20':
...
def train(indices, train_loader, model, prompter, optimizer, scheduler, criterion, epoch, args):
    ...
    for i, (images, target) in enumerate(tqdm(train_loader)):
        ...
        output = softmax(output,dim=-1) #[num_classes*B,num_classes] --max-> [num_classes*B,1] --argmax-->[B,1]
        output_prompt_max,_ = torch.max(output,dim=-1)

        output_prompt_index = torch.argmax(output_prompt_max.view(len(indices),-1,1),dim=0)

        base_index = torch.arange(images.size(0)).to(device)
        output_prompt_index = output_prompt_index*images.size(0)+base_index.view(-1,1)
        # print(output_prompt_index.shape)
        output = output[output_prompt_index,:].squeeze()
...
def validate(indices, val_loader, model, prompter, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(tqdm(val_loader)):
            ...
            output_prompt = softmax(output_prompt,dim=-1) #[num_classes*B,num_classes] --max-> [num_classes*B,1] --argmax-->[B,1]
            output_prompt_max,_ = torch.max(output_prompt,dim=-1)
            ...
            output_prompt = output_prompt[output_prompt_index,:].squeeze()
