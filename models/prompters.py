...
class PadPrompter(nn.Module):
    ...
    def forward(self, x):
        ...
        return x + prompt
class PadPrompter_class(nn.Module):
    def __init__(self, args):
        super(PadPrompter_class, self).__init__()
        pad_size = args.prompt_size
        image_size = args.image_size
        self.num_classes =args.num_classes
        self.base_size = image_size - pad_size*2
        self.pad_up = nn.Parameter(torch.randn([self.num_classes, 3, pad_size, image_size]))
        self.pad_down = nn.Parameter(torch.randn([self.num_classes, 3, pad_size, image_size]))
        self.pad_left = nn.Parameter(torch.randn([self.num_classes, 3, image_size - pad_size*2, pad_size]))
        self.pad_right = nn.Parameter(torch.randn([self.num_classes, 3, image_size - pad_size*2, pad_size]))

    def forward(self, x):
        base = torch.zeros(self.num_classes, 3, self.base_size, self.base_size).cuda()
        prompt = torch.cat([self.pad_left, base, self.pad_right], dim=3)
        prompt = torch.cat([self.pad_up, prompt, self.pad_down], dim=2)
        prompt = torch.cat(x.size(0) * [prompt])
        x = torch.cat(self.num_classes * [x])
        return x + prompt
...
def padding_class(args):
    return PadPrompter_class(args)